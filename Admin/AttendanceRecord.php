...
<?php 
	...
	$start = ($page - 1) * $limit;
	$result = $conn->query("SELECT * FROM attendance LIMIT $start, $limit");
	$students = $result->fetch_all(MYSQLI_ASSOC);

	$result1 = $conn->query("SELECT count(name) AS name FROM attendance");
	$studCount = $result1->fetch_all(MYSQLI_ASSOC);
	$total = $studCount[0]['name'];
                ...
                <div style="height: 600px; overflow-y: auto;">
                    <table id="" class="table table-striped table-bordered">
                        ...
                        <tbody id="queryTable">
                            ...
                            <tr>
                                <td><?= $student['attenID']; ?></td>
                                <td><?= $student['name']; ?></td>
                                <td><?= $student['time']; ?></td>
                                <td><?= $student['lectureID']; ?></td>
                                <td><?= $student['subjectID']; ?></td>
                                <td><?= $student['subjectname']; ?></td>
                                <td><a href="AttendanceRecord.php?showinfo=<?= $student ['name']; ?>"><button class="view">View</button></a></td>
                                <td><a href="AttendanceRecord.php?deletedata=<?= $student ['attenID']; ?>"><button class="dlt">Delete</button></a></td>
                            </tr>
                            <?php endforeach;
                             if (isset($_GET['showinfo'])) {
                                $_SESSION['showinfo'] = $_GET['showinfo'];
                                echo "<script> location.replace('info.php'); </script>";
                                }
                            if (isset($_GET['deletedata'])) {
                            $_SESSION['deletedata'] = $_GET['deletedata'];
                            $id = $_SESSION['deletedata'];


                            $sql = "DELETE FROM attendance WHERE attenID = $id;";
                    ...
                    <div class="row">
                        <div class="transparent">
                            <nav aria-label="Page navigation">
                                <ul class="pagination">
                                    <li>
                                        <a href="AttendanceRecord.php?page=<?= $Previous; ?>" aria-label="Previous">
                                            <span aria-hidden="true">&laquo; Previous</span>
                                        </a>
                                    </li>
                                    <?php for($i = 1; $i<= $pages; $i++) : ?>
                                    <li><a href="AttendanceRecord.php?page=<?= $i; ?>"><?= $i; ?></a></li>
                                    <?php endfor; ?>
                                    <li>
                                        <a href="AttendanceRecord.php?page=<?= $Next; ?>" aria-label="Next">
